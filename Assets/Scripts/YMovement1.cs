...
public class YMovement : MonoBehaviour
{
    ...
    {
        isMoving = true;
        ychange = new Vector3(0.0f, 0.03f, 0.0f);  //make a new vector3 that is up 0.005 units

        if (isup == true) //if it's going up
        {
            if (transform.position.y >= 0.8) //if it is above the highest point
            {
        ...
        {
            if (transform.position.y <= -1.1) //if it is below the lowest point
            {
